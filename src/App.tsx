...
class App extends React.Component<AppProps, AppState> {
  ...
  renderWorkspaceSidebar = (history: any, url: string, site: string | null | undefined, workspace: string | null | undefined) => {
    return <WorkspaceSidebar key={url} siteKey={site ? decodeURIComponent(site) : null} workspaceKey={workspace ? decodeURIComponent(workspace) : null} history={history} hideItems={!this.state.forceShowMenu && !this.state.menuIsLocked} menuIsLocked={this.state.menuIsLocked} onToggleItemVisibility={() => {this.toggleForceShowMenu();}} onLockMenuClicked={() => {this.toggleMenuIsLocked();}} />;
  };

  getExtraItems() {
    let items = [<MenuItem primaryText="Reload" onClick={() => {window.location = window.location;}} />, <MenuItem primaryText="Restart Application" onClick={() => {const app = window.require('electron').remote.app;app.relaunch();app.exit(0);}} />];
    return items;
    ...
    return <Switch>
      <Route path="/forms-cookbook" exact={false} render={({
        ...
        if (this.state.configurations && this.state.configurations.global.cookbookEnabled) {
          items.push(<MenuItem primaryText="Exit Cookbook" onClick={() => {history.push('/');}} />);
        }
      ...
      <Route path="*" render={({
        ...
        if (this.state.configurations && this.state.configurations.global.cookbookEnabled) {
          items.push(<MenuItem primaryText="Forms Cookbook" onClick={() => {history.push('/forms-cookbook');}} />);
        }
        return <ExtraOptions items={items} />;
      }} />
    </Switch>;
  ...
  getMenuSwitch() {
    return <Switch>
      <Route path='/sites/:site/workspaces/:workspace' render={({
        match,
        history
      }) => {
      ...
      <Route path="/forms-cookbook" exact={false} render={({
        ...
        return <FormsCookbookSidebar menus={[]} hideItems={!this.state.forceShowMenu && !this.state.menuIsLocked} menuIsLocked={this.state.menuIsLocked} onToggleItemVisibility={() => {this.toggleForceShowMenu();}} onLockMenuClicked={() => {this.toggleMenuIsLocked();}} />;
      }} />
    </Switch>;
  }
  ...
  getContentSwitch() {
    return <Switch>
      ...
      <Route path='/sites/:site/workspaces/:workspace' exact render={({
        match
      }) => {
        //$FlowFixMe
        return <Home key={'home'} siteKey={decodeURIComponent(match.params.site)} workspaceKey={decodeURIComponent(match.params.workspace)} />;
      }} />
      <Route path='/sites/:site/workspaces/:workspace/collections/:collection' exact render={({
        match
      }) => {
        //$FlowFixMe
      ...
      <Route path='/sites/:site/workspaces/:workspace/collections/:collection/:item' exact render={({
        match
      }) => {
        //$FlowFixMe
      ...
      <Route path='/sites/:site/workspaces/:workspace/singles/:single' exact render={({
        match
      }) => {
        //$FlowFixMe
  ...
  render() {
    ...
    let header = <Header minimizeHandler={this.minimizeWindow.bind(this)} toggleMaximizeHandler={this.toggleWindowMode.bind(this)} closeHandler={this.closeWindow.bind(this)} isMaximized={this.state.maximized} />;
