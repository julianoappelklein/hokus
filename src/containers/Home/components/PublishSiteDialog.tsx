...
export default class PublishSiteDialog extends React.Component<PublishSiteDialogProps, PublishSiteDialogState> {
  ...
  render() {
    ...
    const actions = [
      <FlatButton label="Cancel" primary={false} onClick={this.handleCancelClick.bind(this)} />,
      <FlatButton
        disabled={!valid}
    ...
    return (
      <Dialog title="Publish Site" contentStyle={{ maxWidth: "600px" }} open={open} actions={actions}>
        ...
        >
          {this.props.workspace.build.map((build, i) => (
            <MenuItem key={`build-${i}`} value={i} primaryText={build.key} secondaryText={build.config} />
          ))}
        </SelectField>
        <SelectField
