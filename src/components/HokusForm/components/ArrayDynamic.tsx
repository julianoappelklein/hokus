...
import { DynamicFormNode } from "../../HoForm";
...
export default class ArrayDynamic extends BaseDynamic<ArrayDynamicField, ArrayDynamicState> {
  normalizeState(x: NormalizeStateInput<ArrayDynamicField>) {
  ...
  buildBreadcumbFragment(node: any, buttons: Array<{ label: string; node: any }>) {
    if (node.uiState !== undefined && node.uiState.index !== undefined) {
      let arrayTitleField = node.field.fields.find((x: { arrayTitle: any }) => x.arrayTitle);
      let itemTitle;
      if (arrayTitleField) itemTitle = node.state[node.field.key][node.uiState.index][arrayTitleField.key];
      else itemTitle = node.uiState.index + "";
    ...
    if (currentPath === context.nodePath) {
      return (
        <ArrayListManager
          ...
          onItemClickHandler={(e: any, index: any) => {
