...
export const MultiFormatDataDisplay = React.memo((props: {data: any}) => {
    ...
    return (<Tabs value={format} onChange={(value: string) => setFormat(value)}>
        <Tab label="YAML" value="yaml">
            {format === 'yaml' && (<PreFormatedCode>
                {jsYaml.dump(props.data)}
            </PreFormatedCode>)}
        </Tab>
        <Tab label="TOML" value="toml">
            {format === "toml" && <PreFormatedCode>
                {tomlify.toToml(props.data, { space: 2 })}
            </PreFormatedCode>}
        </Tab>
        <Tab label="JSON" value="json">
            {format === "json" && (<PreFormatedCode>
