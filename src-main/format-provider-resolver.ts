...
${content}`;
  ...
  parseFromMdFileString(str: string) {
    ...
    let jsonEnd = -1;
    if (jsonExecResult != null) jsonEnd = jsonExecResult.index;
...
class FormatProviderResolver {
  ...
  _getFileLinePromise(filename: string, line_no: number): Promise<string | undefined> {
    return new Promise((resolve, reject) => {
      ...
      stream.on("data", function(data: any) {
        fileData += data;
        // The following lines should be improved
  ...
  resolveForMdFirstLine(line: string) /*: ?FormatProvider*/ {
    ...
    for (let i = 0; i < this._exts.length; i++) {
      ...
      if (f.matchContentFirstLine(line)) return f;
    }
    return undefined;
  }
  ...
  resolveForMdFilePromise(filePath: string) /*: Promise<?FormatProvider>*/ {
    return this._getFileLinePromise(filePath, 0).then(line => {
      if (line != null) return Promise.resolve(this.resolveForMdFirstLine(line));
